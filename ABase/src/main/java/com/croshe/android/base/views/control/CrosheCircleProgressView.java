...
public class CrosheCircleProgressView extends ProgressBar {

    private int mReachBarSize = DensityUtils.dip2px(2); // 未完成进度条大小
    private int mNormalBarSize = DensityUtils.dip2px(2); // 未完成进度条大小
    private int mReachBarColor = Color.parseColor("#108ee9"); // 已完成进度颜色
    private int mNormalBarColor = Color.parseColor("#FFD3D6DA"); // 未完成进度颜色
    private int mTextSize = DensityUtils.sp2px(14); // 进度值字体大小
    ...
    private int mOuterSize = DensityUtils.dip2px( 1); // 外层圆环宽度
    ...
    public CrosheCircleProgressView(Context context) {
        this(context, null);
    }

    public CrosheCircleProgressView(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public CrosheCircleProgressView(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void obtainAttributes(AttributeSet attrs) {
        TypedArray ta = getContext().obtainStyledAttributes(attrs, R.styleable.CrosheCircleProgressView);
        mProgressStyle = ta.getInt(R.styleable.CrosheCircleProgressView_cpv_progressStyle, ProgressStyle.NORMAL);
        // 获取三种风格通用的属性
        mNormalBarSize = (int) ta.getDimension(R.styleable.CrosheCircleProgressView_cpv_progressNormalSize, mNormalBarSize);
        mNormalBarColor = ta.getColor(R.styleable.CrosheCircleProgressView_cpv_progressNormalColor, mNormalBarColor);

        mReachBarSize = (int) ta.getDimension(R.styleable.CrosheCircleProgressView_cpv_progressReachSize, mReachBarSize);
        mReachBarColor = ta.getColor(R.styleable.CrosheCircleProgressView_cpv_progressReachColor, mReachBarColor);

        mTextSize = (int) ta.getDimension(R.styleable.CrosheCircleProgressView_cpv_progressTextSize, mTextSize);
        mTextColor = ta.getColor(R.styleable.CrosheCircleProgressView_cpv_progressTextColor, mTextColor);
        mTextSkewX = ta.getDimension(R.styleable.CrosheCircleProgressView_cpv_progressTextSkewX, 0);
        if (ta.hasValue(R.styleable.CrosheCircleProgressView_cpv_progressTextSuffix)) {
            mTextSuffix = ta.getString(R.styleable.CrosheCircleProgressView_cpv_progressTextSuffix);
        }
        if (ta.hasValue(R.styleable.CrosheCircleProgressView_cpv_progressTextPrefix)) {
            mTextPrefix = ta.getString(R.styleable.CrosheCircleProgressView_cpv_progressTextPrefix);
        }
        mTextVisible = ta.getBoolean(R.styleable.CrosheCircleProgressView_cpv_progressTextVisible, mTextVisible);

        mRadius = (int) ta.getDimension(R.styleable.CrosheCircleProgressView_cpv_radius, mRadius);
        ...
        switch (mProgressStyle) {
            ...
            case ProgressStyle.FILL_IN_ARC:
                mStartArc = ta.getInt(R.styleable.CrosheCircleProgressView_cpv_progressStartArc, 0) + 270;
                mInnerPadding = (int) ta.getDimension(R.styleable.CrosheCircleProgressView_cpv_innerPadding, mInnerPadding);
                mOuterColor = ta.getColor(R.styleable.CrosheCircleProgressView_cpv_outerColor, mReachBarColor);
                mOuterSize = (int) ta.getDimension(R.styleable.CrosheCircleProgressView_cpv_outerSize, mOuterSize);
                mReachBarSize = 0;// 将画笔大小重置为0
                mNormalBarSize = 0;
                if (!ta.hasValue(R.styleable.CrosheCircleProgressView_cpv_progressNormalColor)) {
            ...
            case ProgressStyle.NORMAL:
                mReachCapRound = ta.getBoolean(R.styleable.CrosheCircleProgressView_cpv_reachCapRound, true);
                mStartArc = ta.getInt(R.styleable.CrosheCircleProgressView_cpv_progressStartArc, 0) + 270;
                if (ta.hasValue(R.styleable.CrosheCircleProgressView_cpv_innerBackgroundColor)) {
                    mInnerBackgroundColor = ta.getColor(R.styleable.CrosheCircleProgressView_cpv_innerBackgroundColor, Color.argb(0, 0, 0, 0));
    ...
    public void setReachBarSize(int reachBarSize) {
        mReachBarSize = DensityUtils.dip2px( reachBarSize);
        invalidate();
    }
    ...
    public void setNormalBarSize(int normalBarSize) {
        mNormalBarSize = DensityUtils.dip2px( normalBarSize);
        invalidate();
    }
    ...
    public void setRadius(int radius) {
        mRadius = DensityUtils.dip2px( radius);
        invalidate();
    }
    ...
    public void setInnerPadding(int innerPadding) {
        mInnerPadding = DensityUtils.dip2px( innerPadding);
        int mInnerRadius = mRadius - mOuterSize / 2 - mInnerPadding;
        rectInner = new RectF(-mInnerRadius, -mInnerRadius, mInnerRadius, mInnerRadius);
        invalidate();
    }
    ...
    public void setOuterSize(int outerSize) {
        mOuterSize = DensityUtils.dip2px( outerSize);
        invalidate();
    }
